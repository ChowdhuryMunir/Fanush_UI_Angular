...
export class InterviewService {
  ...
  getAllInterviews(): Observable<Interview[]> {
    return this._http.get<Interview[]>(`${this.url}`);
  }

  getInterviewById(id: number): Observable<Interview> {
    return this._http.get<Interview>(`${this.url}/${id}`);
  }

  createInterview(interview: Interview): Observable<Interview> {
    const httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) };
    return this._http.post<Interview>(this.url, interview, httpOptions);
  }

  updateInterview(id: number, interview: Interview): Observable<Interview> {
    const httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) };
    return this._http.put<Interview>(`${this.url}/${id}`, interview, httpOptions);
  }
